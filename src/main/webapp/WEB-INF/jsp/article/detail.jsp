		...
		<tbody>
			...
			<c:if test="${article.extra.file__common__attachment['3'] != null}">
				<tr>
					...
					<td>
						<div class="img-box">
							<img src="/usr/file/showImg?id=${article.extra.file__common__attachment['3'].id}&updateDate=${article.extra.file__common__attachment['3'].updateDate}" alt="image not supported" />
						</div>
					</td>
				</tr>
			</c:if>
		...
		for ( var fileNo = 1; fileNo <= 2; fileNo++ ) {
			...
			if ( fileNo == 3 ) {
				...
				if ( reply.extra.file__common__attachment && reply.extra.file__common__attachment[fileNo] ) {
					...
					html += '<img src="/usr/file/showImg?id=' + file.id + '&updateDate=' + file.updateDate + 'alt="image not supported"/>';
		        }
				else {
				}
